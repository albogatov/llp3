...
enum file_op_status page_header_write_real(FILE *file, struct database_header* database_header, struct relation_header* relation_header) {
    ...
    if (!is_enough_space(real_page, sizeof(struct relation_header))) {
    ...
    if (fwrite(relation_header, sizeof(struct relation_header), 1, file) == 1) {
        real_page->remaining_space -= sizeof(struct relation_header);
        real_page->write_ptr += sizeof(struct relation_header);
...
enum file_op_status relation_header_save_changes(FILE *file, struct relation_header* relation_header) {
    fseek(file, sizeof(struct database_header) + sizeof(struct page_header) + (relation_header->real_number - 1) * sizeof(struct relation_header), SEEK_SET);
    if (fwrite(relation_header, sizeof(struct relation_header), 1, file) == 1) {
        return OK;
    } else return ERROR;
}
...
bool is_relation_present(FILE *file, const size_t length, const char* name, struct relation_header* relation_header) {
    ...
    if (length > 0) {
        ...
        while (idx < length) {
            ...
            fread(relation_header, sizeof(struct relation_header), 1, file);
            ...
            ptr += sizeof(struct relation_header);
