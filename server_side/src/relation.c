...
void row_insert(struct row* row) {
    ...
    enum file_op_status result = row_write_to_page(row->relation->relation_header->database->source_file,
                                                   page_number, row);
    if (result != OK) {
...
char * row_select(struct query *query, bool show_output, char *buf) {
    ...
    for (size_t i = 0; i < query->relation->schema->count; i++) {
        if (strcmp(query->relation->schema->start[i].name, query->name[0]) == 0) {
            ...
            content_type = query->relation->schema->start[i].content_type;
            break;
        }
    }
...
char * row_update(struct query *query, bool show_output, char *buf) {
    ...
    for (size_t i = 0; i < query->relation->schema->count; i++) {
        if (strcmp(query->relation->schema->start[i].name, query->name[0]) == 0) {
            ...
            type_one = query->relation->schema->start[i].content_type;
            ...
            type_two = query->relation->schema->start[i].content_type;
